...
def main():
    ...
    if args.mode == "train":
        ...
        columns_list = ["in_size","out_size","lstm_size","num_layers","batch_size","time_steps","learning_rate","no_batches","time_taken"]

        # afterRun_data=pd.DataFrame(columns=columns_list)
        # afterRun_data.to_csv("hyperparametersEffects.csv",index = False)

        values_list = [in_size,out_size,lstm_size,num_layers,batch_size,time_steps,lr,NUM_TRAIN_BATCHES,duration]

        row=pd.Series(values_list,columns_list)

        temp_df = pd.read_csv("hyperparametersEffects.csv")

        temp_df = temp_df.append([row],ignore_index=True)

        temp_df.to_csv("hyperparametersEffects.csv",index = False)
        #
        ...
        #myAddition
        print(gen_duration)
        myFile=open("createdText.txt","a")
        myFile.write("\n\n")
        myFile.write(str(datetime.datetime.now()))
        myFile.write("\n\n")
        myFile.write(gen_str)
        myFile.write("\n\n")
        # myFile.write(gen_duration)
        # myFile.write("\n\n")
        myFile.close()
        #
